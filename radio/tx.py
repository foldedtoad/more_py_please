...
def initializeRadio():
    ...
    # set data length values
    reg = 0x00040000 + (ustruct.calcsize('hhhh') << 8) + (ustruct.calcsize('hhhh'))
    machine.mem32[NRF_RADIO___PCNF1] = reg
...
def start():
    ...
    while (True):
        if (machine.mem32[NRF_RADIO___STATE] != 11):  # if radio no longer in TX state, then it must have sent a packet
            utime.sleep_ms(200)  # wait 0.5 second before sending next packet
            ...
            radioBuffer = ustruct.pack('hhhh', count, x_val, y_val, z_val)

            radioBuffer_address = uctypes.addressof(radioBuffer)

            print("sent data:", ustruct.unpack('hhhh', radioBuffer))

            #machine.mem32[radioBuffer_address] = ((machine.mem32[radioBuffer_address] + 1) % 0x10000)  # increment the payload value to send something different
